...
当前源码仓库已按公开发布边界整理，但正式公开前仍需完成上游条款复核、GitHub Actions 审计和二进制资产公开策略确认。仓库不包含成品 `.app`、`app.asar`、官方 Codex 安装包、真实 API key、登录态、日志、会话、缓存、SQLite 数据库或第三方二进制。
...
如果你只是想下载后直接使用，请下载维护者发布的私有 Release 成品包。二进制成品包在上游条款复核批准前只作为 private preview 分发，不能跟随公开仓库自动变成 public release asset。面向普通用户的推荐文件名形如：
