...
> ### 🆕 重磅更新：你在 Codex 的全部历史，原封不动搬过来
>
> 以前换工具，就像搬家时把箱子全"忘"在老房子里 —— 历史对话统统留在原版 Codex，搬过来两手空空。**现在不一样了。** DeepCodeX 会把你在原版 Codex 里的**全部**历史搬过来：对话文件、归档线程、历史索引、连线程数据库都一起导，新对话还在后台持续同步。那些凌晨两点的 debug 崩溃现场、那个你发誓一定会写完的项目，统统还在，接着唠就行，谁都别想被落在门口。→ [直接跳过去看](#继续-codex-里的项目)
...
1. 先安装官方 Codex：https://openai.com/codex/
2. 在 GitHub 下载本仓库源码 zip，并完整解压。
3. 双击 `Install-DeepCodeX.command`。

安装器会自动检测 Codex。没有 Codex 时会停止并打开官方下载页；检测通过后，只需要按提示填写 DeepSeek base URL 和 API key。
...
DeepCodeX 还会同步 Codex 左侧栏的项目/对话状态：Codex 里左侧显示的项目和
普通对话会排在前面，DeepCodeX 自己原有的对话和项目会继续保留在后面。
...
> **小提示：** 导入是在后台进行的，历史对话不会一次性全列在页面上。你需要在 DeepCodeX 里搜索某条对话、点开它，DeepCodeX 才会把它加载出来显示。所以一开始列表看着空也别慌，不是没导进来，搜一下你要找的那条就有了。
