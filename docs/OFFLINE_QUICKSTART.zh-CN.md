...
`runtime-external` 包只适合维护者或已经有兼容 runtime 的机器，普通新用户不要优先使用。
...
3. 如果你会使用终端，可以先校验：
   ...
   ```bash
   shasum -a 256 -c DeepCodeX-private-runtime-bundled-*.zip.sha256
   ```
...
说明你拿到的是 `runtime-external` 包。它可以安装应用外壳，但普通新用户还不能直接发起模型请求。请向维护者索要 `runtime-bundled` 包，或让维护者补齐兼容 runtime。
