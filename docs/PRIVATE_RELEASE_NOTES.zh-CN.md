...
`DeepCodeX-private-runtime-external-*.zip` 不包含本地转发 runtime。它适合维护者或已经有兼容 runtime 的机器，不适合普通新用户单独使用，也不会作为默认推荐下载项。
...
如果 macOS 阻止打开，请先确认 `.sha256` 校验是 `OK`，再右键打开安装脚本。仍被拦截时，只对校验通过的解压目录执行 `xattr -dr com.apple.quarantine DeepCodeX-private-runtime-bundled-*`。

## 安全边界

- 安装脚本不会打印 API key。
- 包审计会检查真实本机 key、运行态数据库、日志、会话文件和维护者路径。
- `runtime-bundled` 包包含本地 runtime，只适合私有、已审阅的分发场景。
